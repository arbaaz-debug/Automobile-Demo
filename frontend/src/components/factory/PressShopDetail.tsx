...
import { useSnapshot, useNow } from "@/hooks/useSnapshot";
...
/**
 * Station-level press-shop detail for one factory.
 *
 * This is the only process in the portal with real instrumentation behind it,
 * so it is the only one that earns a 3D line, a station matrix and per-machine
 * health. It renders as a section of the press-shop process page rather than as
 * a page of its own — the page supplies the shell, the breadcrumbs and the
 * model context, and this supplies the press shop.
 *
 * Counts here are **panels**, not vehicles: this is the press shop's own view
 * and a press makes panels. The page above converts to vehicles for its
 * headline, and says so.
 */
export function PressShopDetail({
  factoryId,
  dateIso,
  shiftId,
}: {
  ...
  const { snapshot } = useSnapshot(dateIso, shiftId);
  const now = useNow();

  const plant = snapshot?.plants.find((p) => p.plantId === factoryId) ?? null;
  ...
  const selectedStation = useMemo(() => {
    ...
    return (
      activeLine.stations.find((s) => s.def.kind === "draw" || s.def.kind === "blanking") ??
      activeLine.stations[0] ??
      null
    );
  }, [activeLine, stationId]);

  if (!snapshot || !plant || !activeLine) return <PageSkeleton inline />;
